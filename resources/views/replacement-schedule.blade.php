@extends('master')
@section('title', 'Replacement Schedule')
@section('include')
    ...
    <script type="text/javascript" class="init">
        $(function () {
            ...
            //always fetch the latest data from database in ReplacementScheduleController and ReplacementSchedule Model
            const table = $('#replacement-schedule').dataTable( {
                "aaData": JSON.parse('{!! $replacement_schedule_content !!}'),
                ...
                buttons: [
                    ...
                    @can('replacement-schedule-update')
                    ...
                    {className: 'btn-sm btn-info', text: 'Save', name: 'save', action: function ( e, dt, node, config ) {
                            post('/replacement-schedule/update', {_token: '{{csrf_token()}}', content: JSON.stringify(dt.rows().data().toArray())});
                        }}
                    @endcan
                ],
                "aoColumns": [
...
@endsection
@section('content')
    <?php $replacement_schedule = json_decode($replacement_schedule_content, true); ?>
    <div class="container">
        <table role="grid" class="table table-bordered display" id="replacement-schedule">
            <thead>
            <tr>
                <th>Date</th>
                <th>Class</th>
